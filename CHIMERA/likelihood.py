...
class hyperlikelihood(object):
  ...
  Args:
    ...
    z_grids (jnp.ndarray): Redshift integration used for integration.
  ...
  def __init__(
    ...
    self.adaptive_z_grids = adaptive_z_grids
  ...
  def p_gw1d(self, pop_lambdas):
    ...
    if self.adaptive_z_grids:
      return self._p_gw1d_adaptive(pop_lambdas)
    else:
      return self._p_gw1d_static(pop_lambdas)

  def _p_gw1d_static(self, pop_lambdas):
    r"""p_gw1d using the static pre-computed z_grids (shape: nevents, z_int_res)."""
    ...
    if self.kind_kde == 'binned':
      ...
      _p_gw = kde_vec(self.z_grids, th_src.z, weights, self.kde_bw, self.kernel, self.num_bins, self.cut_grid)
    elif self.kind_kde == 'fft':
      kde_vec = jax.vmap(fft_kde1d, in_axes=(0,0,0,None,0))
      _p_gw = kde_vec(self.z_grids, th_src.z, weights, self.kde_bw, self.z_grids_edges)
    else:
      kde_vec = jax.vmap(kde1d, in_axes=(0,0,0,None,None))
      _p_gw = kde_vec(th_src.z, self.z_grids, weights, self.kernel, self.kde_bw)

    p_gw = jnp.where(n_effs[:,None]>self.pe_neff, _p_gw, 0.0)*norms[:,None]
    return p_gw, self.z_grids

  def _p_gw1d_adaptive(self, pop_lambdas):
    r"""p_gw1d with per-event grids recomputed each call from the H0-dependent z samples.

    Grid bounds: [max(data_min - cut_grid*sigma, eps), data_max + cut_grid*sigma]
    """
    th_src, weights = get_theta_src_and_weights(pop_lambdas, self.theta_gw_det)

    # Per-event adaptive grids
    data_min = jnp.min(th_src.z, axis=-1)   # (nevents,)
    data_max = jnp.max(th_src.z, axis=-1)   # (nevents,)
    sigma    = jnp.std(th_src.z, axis=-1)   # (nevents,)
    lb = jnp.maximum(data_min - self.cut_grid * sigma, 1.e-8)
    ub = data_max + self.cut_grid * sigma
    eff_grids = jax.vmap(lambda a, b: jnp.linspace(a, b, self.z_int_res))(lb, ub)  # (nevents, z_int_res)
    ...
    if self.kind_kde == 'binned':
      kde_vec = jax.vmap(binned_kde1d, in_axes=(0,0,0,None,None,None,None))
      _p_gw = kde_vec(eff_grids, th_src.z, weights, self.kde_bw, self.kernel, self.num_bins, 0)
    elif self.kind_kde == 'fft':
      eff_grids_edges = jax.vmap(build_hist_edges)(eff_grids)
      kde_vec = jax.vmap(fft_kde1d, in_axes=(0,0,0,None,0))
      _p_gw = kde_vec(eff_grids, th_src.z, weights, self.kde_bw, eff_grids_edges)
    else:
      kde_vec = jax.vmap(kde1d, in_axes=(0,0,0,None,None))
      _p_gw = kde_vec(th_src.z, eff_grids, weights, self.kernel, self.kde_bw)
  ...
  def p_gw3d_single1d(self, pop_lambdas):
    ...
    p_gw3d = p_gw1d[:,None,:] * self.gw_loc2d_pdf[:,:,None] # (Nevents, MaxPixels, ResGrids)
    return p_gw3d
  ...
  def p_gw3d_many1d(self, pop_lambdas):
    ...
    def p_gw_single_event(ev):
      ...
      bw = scott_bw1d(th_src.z[ev], weights[ev]) if self.kde_bw is not None else self.kde_bw
      # Pixels loop
      def pixels_loop(pix, acc):
        pixel_mask = pe_pix == pixels[pix]
        z_pix = jnp.where(pixel_mask, th_src.z[ev], jnp.nan) # before there was jnp.min(z) instead of NaN
        w_pix = jnp.where(pixel_mask, weights[ev], 0.0)
        if self.kind_kde == 'many-1d-binned':
          kde = binned_kde1d(self.z_grids[ev], z_pix, w_pix, bw, self.kernel, self.num_bins, self.cut_grid)
        elif self.kind_kde == 'many-1d-fft':
          kde = fft_kde1d(self.z_grids[ev], z_pix, w_pix, bw=bw, bin_edges=self.z_grids_edges[ev])
        else:
          kde = kde1d(z_pix, self.z_grids[ev], w_pix, kernel=self.kernel, bw=bw)
    ...
    return jnp.where(n_effs[:,None,None]>=self.pe_neff, pgw, jnp.zeros((self.nevents,  self.theta_gw_det.max_npixels, self.z_int_res)))
  ...
  def _compute_like_num_evs_pixelated(self, pop_lambdas):
    ...
    integrand = jnp.where(~jnp.isnan(p_z),
      p_gw3d * p_z / jacobian[:,None,:],
      jnp.zeros((self.nevents, self.theta_gw_det.max_npixels, self.z_int_res))
    ) # (Nevents, MaxPixels, ResGrids)
    like_num_evs_pixels = trapz(integrand, self.z_grids[:,None,:], axis = -1) # (Nevents, MaxPixels)
    # Sum pixel contributions
    like_num_evs = jnp.sum(like_num_evs_pixels, axis = -1) # (Nevents,)
    return like_num_evs

  def _compute_like_num_evs_no_pixels(self, pop_lambdas):
    # p_gw(z,| \theta_gw, \lambda_c, \lambda_m)
    # p_gw = self.p_gw1d(pop_lambdas) -- TBD
    p_gw, eff_grids = self.p_gw1d(pop_lambdas)
    # p_z of having a cbc at z
    p_z = p_cbc(pop_lambdas, eff_grids)
    # jacobian
    jacobian = ddLdz_at_z(pop_lambdas.cosmo, eff_grids) * (1.+eff_grids)**2
    # Integral
    # like_num_evs = trapz(p_gw*p_z/jacobian, self.z_grids, axis = -1)
    like_num_evs = trapz(p_gw*p_z/jacobian, eff_grids, axis=-1)
    return like_num_evs
