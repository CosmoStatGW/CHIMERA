...
class flrw(eqx.Module):
    ...
    name       = 'flrw'
    cosmo_keys = ['H0', 'Om0', 'Ok0', 'Or0', 'w0', 'wa']
...
class mg_flrw(flrw):
    ...
    name       = 'mg_flrw'
    cosmo_keys = ['H0', 'Om0', 'Ok0', 'Or0', 'w0', 'wa', 'Xi0', 'n']

    # everything else is inherited from `flrw`
...
@dispatch
@jax.jit
def E_at_z(cosmo: Union[flrw, mg_flrw], 
           z: jnp.ndarray):
...
@dispatch
@jax.jit
def dCr_at_z(cosmo: Union[flrw, mg_flrw], 
             z: jnp.ndarray):
...
def dCt_at_z(cosmo: Union[flrw, mg_flrw], 
    ...
    if distances is not None:
        return dL2dCt(cosmo, distances, z)   

    dCr    = dCr_at_z(cosmo, z)
    sqrtOk0 = jnp.sqrt(jnp.abs(cosmo.Ok0))
    dH      = cosmo.dH
    dCt     = jnp.where(cosmo.Ok0 == 0.0, dCr,
...
@dispatch
@jax.jit
def dVcdz_at_dz(cosmo: Union[flrw, mg_flrw], 
             z: jnp.ndarray, 
             distances: Optional[jnp.ndarray] = None):
...
def ddLdz_at_z (cosmo: mg_flrw, 
    ...
    dLflrw  = dCt*(1.+z)    
    ddLflrw = dCt + (cosmo.dH/E_at_z(cosmo, z))*(1.+z) 
    
    Xiz  = Xi_at_z(cosmo,z)
    dXiz = (cosmo.Xi0-1.)/(1.+z)**(cosmo.n+1)
...
def compute_z_grids(cosmo: Union[flrw, mg_flrw], 
                    ...
                    cosmo_prior: Optional[Dict[str, List[float]]] = None, 
                    z_conf_range: Union[int, list] = 5, 
                    z_int_res: int = 300):
    ...
    # update cosmo prior
    cp = {k:[v, v] for k,v in cosmo.get_fiducial_params().items()}
    ...
    cosmo1 = cosmo.from_params(**lc_low)
    cosmo2 = cosmo.from_params(**lc_high)

    z_min  = z_from_dGW(cosmo1, dL_min, max_z, 2000)
    z_max  = z_from_dGW(cosmo2, dL_max, max_z, 2000)
