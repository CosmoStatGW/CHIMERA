...
def cupy_kde1d(dataset,
  ...
  # Normalize weights
  if weights is None:
    weights = xp.ones_like(dataset) / dataset.size
  else:
    weights = weights / xp.sum(weights)
  # Bw selection
  neff = 1.0 / xp.sum(xp.power(weights, 2))
  if bw_method == "scott" or bw_method is None:
    bandwidth = xp.power(neff, -1. / (1 + 4))
    bandwidth *= xp.std(dataset)
  elif bw_method == "silverman":
    bandwidth = xp.power(neff * (1 + 2) / 4.0, -1. / (1 + 4))
    bandwidth *= xp.std(dataset)
  elif xp.isscalar(bw_method) and not isinstance(bw_method, str):
    bandwidth = bw_method
    bandwidth *= xp.std(dataset)
  else:
    raise ValueError("bw_method should be 'scott', 'silverman', or a scalar")
  ...
  if cut_grid is not None:
    data_min, data_max, sigma = xp.min(dataset), xp.max(dataset), xp.std(dataset)
    lb = data_min - cut_grid*sigma
    if lb<0.: lb = 1.e-6
    ub = data_max + cut_grid*sigma
    _grid = xp.linspace(lb, ub, grid.size // 2)
  else:
    _grid = grid
  # Choose kernel and compute kernel values
  ...
  # Calculate density
  density = xp.sum(weights * kernel_vals, axis=-1) / bandwidth
  # Interpolate density at grid
  return xp.interp(grid, _grid, density, left = 0., right = 0.)

def _epan_kernel_cupy(u):
  return xp.where(xp.abs(u) <= 1, 3/4 * (1 - u**2), 0)

def _gaussian_kernel_cupy(u):
  return xp.exp(-0.5 * u**2) #/ xp.sqrt(2 * xp.pi)
