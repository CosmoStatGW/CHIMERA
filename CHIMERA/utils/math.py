...
@partial(jax.jit, static_argnames=['kernel', 'bw_method'])
def kde1d(dataset,
  grid,
  weights=None,
  kernel='epan',
  ...
  if bw_method == "scott" or bw_method is None:
    ...
    bandwidth = bw_method # suppose it is already multiplied by std(dataa)
    bandwidth *= jnp.std(dataset)
  else:
...
def numba_gkde_nd(dataset, evaluation_grid, weights=None, bw_method=None, in_log=False):
  ...
  d_dataset, n_dataset = dataset.shape
  points = np.atleast_2d(evaluation_grid)
  d_points, n_points = points.shape
  if d_points != d_dataset:
    if d_points == 1 and n_points == d_dataset:
      points = points.T
  ...
  if weights is not None:
    ...
    _weights = np.full(n_dataset, 1.0 / n_dataset, dtype=dataset.dtype)
  # select bw
  neff = 1.0 / np.sum(np.power(_weights,2))
  if bw_method == "scott" or bw_method is None:
    factor = np.power(neff, -1. / (d_dataset + 4))
  elif bw_method == "silverman":
    factor = np.power(neff * (d_dataset + 2) / 4.0, -1. / (d_dataset + 4))
  elif np.isscalar(bw_method) and not isinstance(bw_method, str):
    factor = bw_method
  else:
    raise ValueError("`bw_method` should be 'scott', 'silverman', a scalar")
  # data cov
  _mean     = np.sum(_weights * dataset, axis=1)
  _residual = (dataset - _mean[:, None])
  _data_covariance = np.atleast_2d(np.dot(_residual * _weights, _residual.T))
  _data_covariance /= (1 - np.sum(_weights ** 2))
  _data_inv_cov = np.linalg.inv(_data_covariance)
  covariance  = _data_covariance * factor**2
  inv_cov     = _data_inv_cov / factor**2
  # whitening
  whitening        = np.linalg.cholesky(inv_cov)
  points_whitened  = np.dot(np.ascontiguousarray(points.T), whitening)
  dataset_whitened = np.dot(np.ascontiguousarray(dataset.T), whitening)
  # main computation
  if USE_GPU:
    norm_factor = np.sqrt(np.linalg.det(2*np.pi*covariance))
    chi2 = cdist(points_whitened, dataset_whitened, 'euclidean') ** 2
    if in_log:
      log_res = -0.5 * chi2 + np.log(_weights) - np.log(norm_factor)
      to_ret  = logsumexp(log_res, axis=1)
    else:
      to_ret  = np.sum(np.exp(-0.5 * chi2) * _weights, axis=1) / norm_factor
  else:
...
def cupy_kde1d(dataset,
  ...
  # Normalize weights
  if weights is None:
    weights = np.ones_like(dataset) / dataset.size
  else:
    weights = weights / np.sum(weights)
  # Bw selection
  neff = 1.0 / np.sum(np.power(weights, 2))
  if bw_method == "scott" or bw_method is None:
    bandwidth = np.power(neff, -1. / (1 + 4))
    bandwidth *= np.std(dataset)
  elif bw_method == "silverman":
    bandwidth = np.power(neff * (1 + 2) / 4.0, -1. / (1 + 4))
    bandwidth *= np.std(dataset)
  elif np.isscalar(bw_method) and not isinstance(bw_method, str):
    bandwidth = bw_method
    bandwidth *= np.std(dataset)
  ...
  if cut_grid is not None:
    data_min, data_max, sigma = np.min(dataset), np.max(dataset), np.std(dataset)
    lb = data_min - cut_grid*sigma
    if lb<0.: lb = 1.e-6
    ub = data_max + cut_grid*sigma
    _grid = np.linspace(lb, ub, grid.size // 2)
  else:
    _grid = grid
  # Choose kernel and compute kernel values
  kernel_fn   = _epan_kernel_cupy if kernel == 'epan' else _gaussian_kernel_cupy
  kernel_vals = kernel_fn((_grid[:,None] - dataset) / bandwidth)
  # Calculate density
  density = np.sum(weights * kernel_vals, axis=-1) / bandwidth
  # Interpolate density at grid
  return np.interp(grid, _grid, density, left = 0., right = 0.)

def _epan_kernel_cupy(u):
  return np.where(np.abs(u) <= 1, 3/4 * (1 - u**2), 0)

def _gaussian_kernel_cupy(u):
