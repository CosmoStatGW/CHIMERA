...
def plot_pixelization(pix_cat, ev, cmap=None, ax = None, figsize=(6,4), label=None, plot_grid=False):
  ...
  for i,jpix in enumerate(np.array(pix_idx)):
    ax.scatter(samples[0][pe_pix_idx==jpix], samples[1][pe_pix_idx==jpix], color=cmap[i], alpha=0.25, s=50, marker='x')
    ax.scatter(grid[0][i], grid[1][i], s=100, marker='o', color=cmap[i], edgecolor='black', linewidth=1.)
