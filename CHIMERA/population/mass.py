...
class base_mass_paired_struct(eqx.Module):
  ...
  norm_p_m1: float
  default = {'m_low':5.1, 'm_high':87., 'grid_res':1000}
...
# utility function
def get_normalizations(mass:base_mass_paired_struct):
  setattr(mass, 'm_grid', jnp.logspace(jnp.log10(mass.m_low), jnp.log10(mass.m_high), mass.grid_res))
  p_values = secondary_mass_conditioned_pdf_notnorm(mass, mass.m_grid, mass.m_high)
  cdf_values = cumtrapz(p_values, mass.m_grid)
  setattr(mass, 'cdf_m2_conditioned', cdf_values)
  integrand_values = primary_mass_pdf_notnorm(mass, mass.m_grid)
  norm = trapz(integrand_values, x=mass.m_grid)
  setattr(mass, 'norm_p_m1', norm)

# Specific implementations

class tpl(base_mass_paired_struct):
  ...
  alpha: float
  beta: float
  default = {**base_mass_paired_struct.default, 'alpha':2.5, 'beta':1.1}
  name = 'truncated_power_law'
  # for the tpl case the normalizations can be computed analitically
  # norm_p_m1 = tpl_cdf(alpha, m_low, m_high)
  # cdf_m2_conditioned = tpl_cdf(alpha, m_low, m1)
  # since this model is not used we avoid to redefine these functions

class bpl(base_mass_paired_struct):
  ...
  default = {**base_mass_paired_struct.default, 'alpha_1':1.6, 'alpha_2':5.6, 'beta':1.1, 'delta_m':4.8, 'break_fraction':0.43}
...
class plp(base_mass_paired_struct):
  ...
  mu_g: float
  sigma_g: float
  default = {**base_mass_paired_struct.default, 'lambda_peak':0.039, 'alpha':3.4, 'beta':1.1, 'delta_m':4.8, 'mu_g':34., 'sigma_g':3.6}
  name = 'power_law_plus_peak'

class pl2p(base_mass_paired_struct):
  ...
  mu2_g: float
  sigma2_g: float
  default = {**base_mass_paired_struct.default, 'lambda_peak':0.05, 'lambda1':0.5, 'alpha':3.4, 'beta':1.1, 'delta_m':4.8, 'm_low':5.1, 'm_high':87., 'mu1_g':34., 'sigma1_g':3.6, 'mu2_g':68, 'sigma2_g':3}
  name = 'power_law_plus_double_peak'

# Semi-paramtrice PL+spline model:
# slightly modified w.r.t base_struct_mass to handle spline_basis and spline_coeffs
class pls(eqx.Module):
  alpha: float
  m_low: float
  m_high: float
  delta_m: float
  ...
  def __init__(self, **kwargs):
    ...
    # self.norm_p_m1 = ...missing
    # self.cdf_m2_conditioned = ... missing
...
########################################
# primary mass function not normalized #
########################################
...
@dispatch
def secondary_mass_conditioned_pdf_notnorm(mass:tpl, m2:jnp.ndarray,  m1:Union[Number,jnp.ndarray]):
  return tpl_notnorm(m2, mass.beta, mass.m_low, m1)

@dispatch
def secondary_mass_conditioned_pdf_notnorm(mass:Union[bpl,plp,pl2p], m2:jnp.ndarray, m1:Union[Number,jnp.ndarray]):
